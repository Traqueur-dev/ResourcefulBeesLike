...
public class Updater {
    ...
    public static void checkUpdates() {
        if(!Updater.isUpToDate()) {
            BeeLogger.warning("ResourcefulBeesLike is not up to date, the latest version is " + Updater.fetchLatestVersion());
        }
    }
    ...
    public static String getVersion() {
        return JavaPlugin.getProvidingPlugin(ResourcefulBeesLike.class).getDescription().getVersion();
    }
    ...
    public static String fetchLatestVersion() {
        try {
            URL url = URI.create("https://api.github.com/repos/Traqueur-dev/ResourcefulBeesLike/releases/latest").toURL();
